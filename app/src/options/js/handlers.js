...
// Burp Suite
function handleChangeBurpURL() {
    var burpURL = this.value;
    if (burpURL) {
        try {
            if (!ALLOWED_PROTOCOL.includes(new URL(burpURL).protocol)) {
                errorMessage(`Protocol must be one of: ${ALLOWED_PROTOCOL.join(", ")}!`, window.errorBurp);
                return;
            }
            burpURL = new URL(burpURL).href;
        } catch {
            errorMessage("Invalid URL!", window.errorBurp);
            return;
        }
    }

    window.burpConfig.url = burpURL;
    extensionAPI.storage.local.set({ burpConfig: window.burpConfig });
    errorMessage("Burp Suite URL updated!", window.errorBurp);
}

function handleBurpEnable() {
    if (!window.burpConfig.url) {
        errorMessage("Burp Suite Webhook URL is required!", window.errorBurp);
        return;
    }
    window.burpConfig.enabled = true;
    extensionAPI.storage.local.set({ burpConfig: window.burpConfig });
    errorMessage("Burp Suite integration enabled!", window.errorBurp);
}

function handleBurpDisable() {
    window.burpConfig.enabled = false;
    extensionAPI.storage.local.set({ burpConfig: window.burpConfig });
    errorMessage("Burp Suite integration disabled!", window.errorBurp);
}

function handleBurpTest() {
    const testOutput = document.getElementById("burp-test-output");
    const testResult = document.getElementById("burp-test-result");
    ...
    testOutput.innerText = "Sending test finding to Burp Suite...";

    const testFinding = [{
        "dupKey": "test-" + Date.now(),
        "tag": "TEST",
        "type": "test",
        "sink": "testConnection",
        "data": "DOMLogger++ Connection Test",
        "href": window.location.href,
        "frame": "top",
        "trace": "N/A",
        "date": new Date().toLocaleString(),
        "debug": "Test payload from options page",
        "alert": false
    }];

    fetch(window.burpConfig.url, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify(testFinding)
    })
    .then(response => {
        if (response.ok) {
            testOutput.innerText = "Success! Burp Suite received the test finding.\nStatus: " + response.status + " " + response.statusText;
            testOutput.style.color = "#28a745";
        } else {
            testOutput.innerText = "Error! Burp Suite returned an error.\nStatus: " + response.status + " " + response.statusText + "\n\nMake sure the Burp extension is running and the port is correct.";
            testOutput.style.color = "#dc3545";
        }
    })
    .catch(error => {
        testOutput.innerText = "Connection failed! Could not reach Burp Suite.\nError: " + error.message + "\n\n1. Ensure Burp Suite is running\n2. Ensure DOMLogger++ Burp Extension is installed\n3. Check if the URL is correct and accessible";
        testOutput.style.color = "#dc3545";
    });
}
