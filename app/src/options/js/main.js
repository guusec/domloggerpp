...
const initStorageVariables = () => {
    ...
    window.burpConfig = { url: "http://localhost:8089/webhook", enabled: false };
    extensionAPI.storage.local.get("burpConfig", (data) => {
        if (data.burpConfig) {
            window.burpConfig = data.burpConfig;
        }
        updateUIBurp(window.burpConfig);
    });
            ...
            for (const [key, values] of Object.entries(changes)) {
                switch (key) {
                    ...
                    case "caidoConfig":
                        ...
                        break;
                    case "burpConfig":
                        window.burpConfig = values.newValue;
                        updateUIBurp(window.burpConfig);
                        break;
                }
            }
...
const main = async () => {
    ...
    window.errorBurp = document.getElementById("errorBurp");
    ...
    // Burp Suite
    document.getElementById("burpURL").addEventListener("change", handleChangeBurpURL);
    document.getElementById("burp-enable-btn").addEventListener("click", handleBurpEnable);
    document.getElementById("burp-disable-btn").addEventListener("click", handleBurpDisable);
    document.getElementById("burp-test-btn").addEventListener("click", handleBurpTest);
