const init = () => {
    extensionAPI.storage.local.get(null, (data) => {
        ...
        // Set default burpConfig settings
        if (data.burpConfig === undefined) {
            extensionAPI.storage.local.set({ burpConfig: {
                url: "http://localhost:8089/webhook",
                enabled: false
            }});
        }

        // Set burpConfig attribute after browser restart
        if (data.burpConfig) {
            MessagesHandler.burpConfig = data.burpConfig;
        }
            ...
            for (const [key, values] of Object.entries(changes)) {
                ...
                switch (key) {
                    ...
                    case "caidoConfig":
                        ...
                        break;
                    case "burpConfig":
                        MessagesHandler.burpConfig = values.newValue;
                        break;
                }
            }
